...
//	
	...
	public String certiPagi(@PathVariable(value="pageNo")int pageNo,Model model) {
		int pageSize=3;
		Page<StaffDto>page=serv.certiPagi(pageNo, pageSize);
...
//		List<MDEP001>list=serv.getAll();
	 ...
	 * public ModelAndView setuppositionupdate(@RequestParam("id")String id,
	 * ModelMap model) { Optional<MCTF001> dtoget = serv.getByCode(id); MCTF001
	 * dto1=dtoget.get(); CertifyBean bean = new CertifyBean();
	 * bean.setId(dto1.getId()); bean.setName(dto1.getName());
	 * bean.setSchool(dto1.getSchool()); bean.setCreate(dto1.getCreatedate());
