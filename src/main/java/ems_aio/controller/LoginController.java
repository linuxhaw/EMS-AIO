...
public class LoginController {
	...
	public String login(@ModelAttribute("login") UserBean bean, HttpSession session, ModelMap model) {
		...
		if (list.size() == 0) {
			model.addAttribute("err", "User not found!");
			return "/";
		} else if (bean.getPassword().equals(list.get(0).getEmp_password())) {
			session.setAttribute("sesUser", list.get(0));
			if (list.get(0).getEmp_rol().equals("1")) {
				return "/EMS-DSH-001.html";
			} else if (list.get(0).getEmp_rol().equals("2")) {
				return "/EMS-MRP-003.html";

			}

			else {
				return "/EMS-SRP-003.html";
			}
